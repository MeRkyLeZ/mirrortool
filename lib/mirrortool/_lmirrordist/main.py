...
def parse_args(args):
    description = "lmirrordist - a fetch tool for mirroring " "of package distfiles"
    usage = "lmirrordist [options] <action>"
    parser = argparse.ArgumentParser(description=description, usage=usage)

    actions = parser.add_argument_group("Actions")
    actions.add_argument(
        "--version", action="store_true", help="display mirrortool version and exit"
    )
    actions.add_argument(
        "--mirror",
        action="store_true",
...
def lmirrordist_main(args):
    ...
    if options.version:
        sys.stdout.write(f"MirrorTool {mirrortool.VERSION}\n")
        return os.EX_OK
